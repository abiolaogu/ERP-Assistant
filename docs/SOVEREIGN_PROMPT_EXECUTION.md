# Sovereign Prompt Execution Pack - ERP-Assistant

These prompts operationalize the Sovereign SaaS 2026 guidance for **ERP-Assistant** (Enterprise AI Assistant).

## Prompt 1 - Domain and Tenant Contract

```
You are implementing hard multi-tenancy for ERP-Assistant.
...
```
You are implementing event-native workflows for ERP-Assistant.

Tasks:
1. Define event contracts for: assistant.runs, assistant.approvals, assistant.events.
...
```
You are redesigning ERP-Assistant for category leadership against Microsoft Copilot, Notion AI, Slack AI.
...
```
You are preparing ERP-Assistant for immediate production deployment.

Tasks:
1. Enforce SLO checks (availability 99.95%, p95 <= 250ms).
