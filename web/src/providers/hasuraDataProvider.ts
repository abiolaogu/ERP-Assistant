  ...
  for (const f of filters) {
    if ("field" in f) {
      if (f.operator === "contains") {
        ...
        conditions.push({
          [f.field]: { [mapOperator(f.operator)]: f.value },
        });
      }
    }
  }
...
export const dataProvider: DataProvider = {
  ...
  create: async ({ resource, variables, meta }) => {
    ...
    const mutation = gql`
      mutation ($object: ${table}_insert_input!) {
        insert_${table}_one(object: $object) { ${fields} }
      }
    `;
    const result = await graphqlClient.request<Record<string, any>>(mutation, {
      object: variables,
    });
  ...
  update: async ({ resource, id, variables, meta }) => {
    ...
    const fields = meta?.fields
      ...
      : "id";
    const mutation = gql`
      mutation ($id: String!, $set: ${table}_set_input!) {
        update_${table}_by_pk(pk_columns: { id: $id }, _set: $set) { ${fields} }
      }
    `;
    const result = await graphqlClient.request<Record<string, any>>(mutation, {
      id: String(id),
  ...
  deleteOne: async ({ resource, id, meta }) => {
    ...
    const mutation = gql`
      mutation ($id: String!) {
        delete_${table}_by_pk(id: $id) { ${fields} }
      }
    `;
    const result = await graphqlClient.request<Record<string, any>>(mutation, {
      id: String(id),
