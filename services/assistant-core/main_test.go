...
func createSession(t *testing.T, mux http.Handler, tenant string, payload map[string]any) map[string]any {
	t.Helper()
...
func TestCursorRoundTrip(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		createSession(t, mux, tenant, map[string]any{
			"user_id": "user-" + string(rune('A'+i)),
		})
	}
...
func TestTenantIsolation(t *testing.T) {
	...
	createSession(t, mux, "tenant-A", map[string]any{"user_id": "alice"})
	createSession(t, mux, "tenant-B", map[string]any{"user_id": "bob"})
...
func TestCreateValidation(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]any{"intent": "test"})
	req := httptest.NewRequest(http.MethodPost, basePath, bytes.NewReader(body))
...
func TestUpdateAndDelete(t *testing.T) {
	...
	resp := createSession(t, mux, "t1", map[string]any{"user_id": "del-test"})
