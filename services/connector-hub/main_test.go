...
func createConnector(t *testing.T, mux http.Handler, tenant string, payload map[string]any) map[string]any {
	t.Helper()
...
func TestCursorRoundTrip(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		createConnector(t, mux, tenant, map[string]any{
			"name":           "Conn " + string(rune('A'+i)),
			"service_name":   "svc-" + string(rune('a'+i)),
			"connector_type": "erp_module",
		})
	}
...
func TestTenantIsolation(t *testing.T) {
	...
	createConnector(t, mux, "tenant-A", map[string]any{"name": "A", "service_name": "sA", "connector_type": "erp_module"})
	createConnector(t, mux, "tenant-B", map[string]any{"name": "B", "service_name": "sB", "connector_type": "erp_module"})
...
func TestUpdateAndDelete(t *testing.T) {
	...
	resp := createConnector(t, mux, "t1", map[string]any{"name": "Del", "service_name": "s", "connector_type": "custom"})
